### International Migration Flows in the Americas (1960-2000): Their History and Network Dynamics.
...
# Info on R version and machine that were used to produce the results reported in the paper:
# R version 3.4.1 (2017-06-30)
# Platform: x86_64-w64-mingw32/x64 (64-bit)

# Info on the packages that were used and their specific version: 

library(reshape2)         # version 1.4.2
library(network)          # version 1.13.0
library(ergm)             # version 3.6.0
library(statnet)          # version 2.4
...
library(dplyr)            # version 0.71

#for full replicability, uncomment these lines of code if your version of  ergm or xergm is different from the ones reported above
...
# Packages indirectly used in the analysis (i.e. not directly attached) but required by one
# or more of the packages directly attached:

# gtools_3.5.0       lpSolve_5.6.13     splines_3.4.1      stats4_3.4.1      
# mgcv_1.8-17        rlang_0.1.1        nloptr_1.0.4       glue_1.1.1        
# trust_0.1-7        plyr_1.8.4         bindr_0.1          robustbase_0.92-7 
# stringr_1.2.0      caTools_1.17.1     coda_0.19-1        permute_0.9-4     
# parallel_3.4.1     DEoptimR_1.0-8     Rcpp_0.12.10       KernSmooth_2.23-15
# ROCR_1.0-7         gdata_2.18.0       vegan_2.4-3        lme4_1.1-13       
# gplots_3.0.1       stringi_1.1.5      grid_3.4.1         tools_3.4.1       
# bitops_1.0-6       magrittr_1.5       tibble_1.3.3       cluster_2.0.6     
# pkgconfig_2.0.1    MASS_7.3-47        Matrix_1.2-10      assertthat_0.2.0  
# minqa_1.2.4        R6_2.2.2           boot_1.3-19        igraph_1.1.2      
# nlme_3.1-131       compiler_3.4.1  


############################# Import data#############################################################

## set WD
setwd("C:/Users/diego/Desktop/third_ch/data_&_analysis/input_data_files")
## import data
...
setwd("C:/Users/diego/Desktop/third_ch/data_&_analysis/output_data_files")
...
## working directory
setwd("C:/Users/diego/Desktop/third_ch/data_&_analysis/input_data_files")
## reading WB bilateral migrant stock data 1960-2000
IDs.americas            <-read.csv(file="countries_ids_the_americas.csv",header=TRUE,as.is=TRUE)
...
##save the decade-long migration flow matrices in csv and txt format (uncomment the write functions to export the data)
for(i in 1:length(decades))
{
  net  <-(FLOWS.americas_matrix[[i]])
  setwd("C:/Users/diego/Desktop/third_ch/data_&_analysis/output_data_files")
  write.csv(x=net,file=paste("Americas_flows",decades[i],"matrix.csv",sep="_"),row.names=T)
  setwd("C:/Users/diego/Desktop/third_ch/data_&_analysis/input_data_files/circos_input_data_file")
...
setwd("C:/Users/diego/Desktop/third_ch/data_&_analysis/output_data_files/ERGM_GOF")
...
aici_60s<-((as.numeric(IDs.americas$`1960_AICI`) * 1000) + (as.numeric(IDs.americas$`1965_AICI`) * 1000) + (as.numeric(IDs.americas$`1970_AICI`) * 1000)) / 3
aici_70s<-((as.numeric(IDs.americas$`1970_AICI`) * 1000) + (as.numeric(IDs.americas$`1975_AICI`) * 1000) + (as.numeric(IDs.americas$`1980_AICI`) * 1000)) / 3
aici_80s<-((as.numeric(IDs.americas$`1980_AICI`) * 1000) + (as.numeric(IDs.americas$`1985_AICI`) * 1000) + (as.numeric(IDs.americas$`1990_AICI`) * 1000)) / 3
aici_90s<-((as.numeric(IDs.americas$`1990_AICI`) * 1000) + (as.numeric(IDs.americas$`1995_AICI`) * 1000) + (as.numeric(IDs.americas$`2000_AICI`) * 1000)) / 3
...
dev.off()
X               <-paste(top_destinations,"gof-tergm.pdf",sep="_")
gof.tergm       <-gof(model,nsim=500,MCM.interval=50,
                      MCMC.burnin=10000,MCM.samplesize=5000,ncpus=8)
pdf(X, width = 9, height = 6)
dev.off()
plot(gof.tergm)  #export GOF plots
